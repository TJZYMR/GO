...
func GetDatabase() *gorm.DB {
	// databasename := "tatva"
	// database := "mysql"
	// databasepassword := "zymr@123"
	// databaseurl := "postgres://postgres:" + databasepassword + "@localhost/" + databasename + "?sslmode=disable"
	databaseurl := "tatva:zymr@123@tcp(127.0.0.1:3306)/web?charset=utf8mb4&parseTime=True&loc=Local"
...
func GenerateJWT(email, role string) (string, error) {
	...
	token := jwt.New(jwt.SigningMethodHS256) ///first we will decide the signing algo
	claims := token.Claims.(jwt.MapClaims)   //then we will set the claims by first creating type pf it and then adding claims.
...
func IsAuthorized(handler http.HandlerFunc) http.HandlerFunc { //handlefunc as argument telle that it will check for authorization of that url.
	return func(w http.ResponseWriter, r *http.Request) {
		...
		token, err := jwt.Parse(r.Header["Token"][0], func(token *jwt.Token) (interface{}, error) { //here unsigned token after decryption is dispatched after checking it for valid signature for authentication.
		...
		if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid { //now checking the claims in that token whichever we want to authorise.
			if claims["role"] == "admin" { //if role is set to admin then with setting header with that role we are serveing http with writer and request.
...
//no bearer needed while giving token in header in this code.
